...
const UserConfigEditorRenderer = React.memo<EntityEditorRendererProps<UserConfig>>(({ bufferId, buffer }) => {
  ...
  const onUpdateLang = useCallback(
    (e: React.ChangeEvent<{ value: unknown }>) =>
      dispatch(actions.buffers.update<UserConfig>("UserConfig", bufferId, { lang: e.target.value as Lang })),
    []
  );
  const onUpdateTheme = useCallback(
    (e: React.ChangeEvent<{ value: unknown }>) =>
      dispatch(actions.buffers.update<UserConfig>("UserConfig", bufferId, { theme: e.target.value as Theme })),
    []
  );
